...
RUN_MODE = 'plot';                  % Options: 'test', 'plot' or 'full'
...
if strcmp(RUN_MODE, 'plot')
    ...
    RX_NUM_ML = 3:7:10;                 % Additional receiver counts for ML optimization
    nvar_mlpos = length(RX_NUM_ML);     % Number of variants for ML optimization
    ...
    SNR_dB = repmat((-10:2:20)', 1, max(RX_NUM_DOA, max(RX_NUM_ML)));    % SNR in dB
    ...
    map2d = Map2D([10,10], [90, 90], max(RX_NUM_ML));
    metric = Metric();
    ...
    for itr = 1:ITERATION
        ...
        [pos_rx, aoa_act, rot_abs] = map2d.genRXPos(area_size, pos_tx, max(RX_NUM_ML), RX_RANDOMISED, SAFETY_DISTANCE, ABS_ANGLE_LIM, DOA_RESOLUTION);
        ...
        for ml_idx = 1:nvar_mlpos
            ...
            for idx_snr=1:nvar_snr
                pos_rx_active = pos_rx(1:RX_NUM_ML(ml_idx),:);
                y_received_active = y_centralised(idx_snr, 1:RX_NUM_ML(ml_idx),:);
                progressbar('step'); % Update progress bar
...
if FLAG_PLOT
    ...
    plot_aoa_mlpos(all_errors, SNR_dB, metric, doa_est_methods, ...
        RX_NUM_DOA, RX_NUM_ML, DOA_MODE, DOA_RESOLUTION, OPT_GRID_DENSITY, ...
